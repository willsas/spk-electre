...
final class CriteriaViewController: UIViewController {

    @IBOutlet weak var currentDocumentButton: UIButton!
    @IBOutlet weak var uploadButton: UIButton!
    
    private let viewModel = CriteriaViewModel.make()
    ...
    override func viewDidLoad() {
        ...
        Task { [weak self] in await self?.viewModel.getCriteria() }
    }
    ...
    func currentDocumentButtonDidTapped(_ sender: Any) {
        Task { [weak self] in await self?.viewModel.showCriteria() }
    }
